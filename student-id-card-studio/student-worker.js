  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (url.pathname === '/' || url.pathname === '/health') {
        return jsonResponse({ status: 'ok', version: '10.1', project: projectId }, corsHeaders);
      }
      if (url.pathname === '/api/stats') {
        return jsonResponse({ status: 'ok', version: '10.1' }, corsHeaders);
      }
      ...
      if (url.pathname === '/api/debug') {
        ...
        const res = await fetch(`${baseUrl}:runQuery`, {
          ...
          body: JSON.stringify({
            structuredQuery: {
              from: [{ collectionId: 'students', allDescendants: true }],
              limit: 10,
            }
          })
        });
        ...
        return jsonResponse({
          success: true, version: '10.1', studentsFound: docs.length,
          students: docs.map(d => ({
            path: d.name,
            STUDENT_CODE: d.fields?.STUDENT_CODE?.stringValue || 'MISSING',
      ...
      if (url.pathname === '/verify') {
        ...
        const token = await getAccessToken(env);
        const { student, schoolCfg } = await fetchStudentWithSchool(env, code, token, baseUrl);
        if (!student) return htmlResponse(notFoundPage(code), corsHeaders, 404);
        return htmlResponse(verifiedPage(student, code, schoolCfg), corsHeaders);
      }
      ...
      if (url.pathname === '/api/student' && request.method === 'GET') {
        ...
        const token = await getAccessToken(env);
        const { student } = await fetchStudentWithSchool(env, code, token, baseUrl);
        if (!student) return jsonResponse({ success: false, error: 'Not found', code }, corsHeaders, 404);
        return jsonResponse({ success: true, student }, corsHeaders);
      }

      if (url.pathname === '/api/student' && request.method === 'POST') {
        let body = {};
      ...
      return htmlResponse(errorPage(
        ...
        Available routes:<br>
        <code>GET /verify?code=STUDENT_CODE</code><br>
        <code>GET /api/student?code=STUDENT_CODE</code><br>
        <code>POST /api/student</code> — body: {"code":"UMSW2407"}<br>
...
// ══════════════════════════════════════════════════════════════
//  FETCH STUDENT + SCHOOL CONFIG
// ══════════════════════════════════════════════════════════════
async function fetchStudentWithSchool(env, studentCode, token, baseUrl) {
  const headers = { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' };
  ...
  do {
    ...
    for (const r of docs) {
      ...
      if (!fields) continue;
      const sc   = fields.STUDENT_CODE?.stringValue || '';
      const roll = fields.ROLL_NO?.stringValue || '';
      if (sc === code || roll === code) {
        student = parseFields(fields);
        break;
      }
    }
  ...
  if (!student) return { student: null, schoolCfg: null };
...
//  HTML PAGES
// ══════════════════════════════════════════════════════════════
function verifiedPage(s, code, schoolCfg) {
  ...
  const line1 = schoolCfg?.schoolLine1 || s.SCHOOL_ID || 'SCHOOL';
...
<title>Verified — ${sanitize(s.NAME||'Student')}</title>
...
body{
  ...
  background:linear-gradient(160deg,#0d1b6e,#1a3a8f);
  min-height:100vh;
...
.header{
  background:linear-gradient(135deg,#0d1b6e,#1a3a8f);
...
/* ── Stamp: BOTTOM-RIGHT corner of photo ── */
.stamp-overlay{
  position:absolute;
  bottom:-14px;
  right:-14px;
...
.student-name{
  ...
  color:#0d1b6e;text-align:center;
...
.value{font-size:13px;font-weight:700;color:#0d1b6e;word-break:break-word;}
...
<div class="card">
  ...
  <div class="header">
    ...
    <div class="badge-row">
      <span class="badge">✅ STUDENT IDENTITY VERIFIED</span>
    </div>
  </div>
  ...
  <div class="body">
    ...
    <div class="photo-wrap">
      ${hasPhoto
        ? `<img src="${sanitize(s.PHOTO)}" class="photo"
    ...
    <!-- ══ STUDENT NAME ══ -->
    <div class="student-name">${sanitize(s.NAME||'STUDENT')}</div>

    <!-- ══ INFO FIELDS ══ -->
    <div class="info-grid">
      ${field('Class',         s.CLASS)}
      ${field('Section',       s.SEC)}
      ${field('Roll No.',      s.ROLL_NO)}
      ${field('Date of Birth', s.DOB)}
      ${field("Father's Name", s.FATHERS_NAME)}
      ${field('Phone No.',     s.PHONE_NO)}
      ${field('Student Code',  s.STUDENT_CODE||code)}
      ${field('Session',       s.SESSION)}
      ${field('Address',       s.ADDRESS)}
    </div>

    <!-- ══ PRINCIPAL SIGNATURE — right aligned, no admission number ══ -->
...
<!-- ══ CREDIT ══ -->
<div class="credit">
  Designed by <strong>Hrishav Ranjan</strong> &nbsp;❤️&nbsp; © 2026 · School ID Card Verification System
</div>
...
.code-box{
  background:#fee2e2;padding:8px 16px;border-radius:8px;
  font-weight:700;color:#991b1b;font-size:15px;
...
<div class="card">
  ...
  <p>No record found for student code:</p>
  <div class="code-box">${sanitize(code)}</div>
  <div class="warning">⚠️ This ID may be invalid, or student data has not been uploaded yet.</div>
</div>
<div class="credit">
  Designed by <strong>Hrishav Ranjan</strong> &nbsp;❤️&nbsp; © 2026 · School ID Card Verification System
</div>
